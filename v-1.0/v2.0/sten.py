...
def get_farmer(farmer_id):
    ...
    farmer = Users.query.get_or_404(farmer_id)
    if farmer.is_farmer == True:
...
def create_farmer():
    if 'loggedin' in session:
        farmer = Users.query.get_or_404(session['id'])
...
def edit_farmer(farmer_id):
    ...
    if farmer['is_farmer'] == True:
        ...
        if request.method == 'POST':
            ...
            # farmer_ = Users.query.get_or_404(farmer.id)
            return render_template('farmer.html', farmer=farmer)
...
def get_investor(investor_id):
    investor_id = session['id']
    investor = Users.query.filter(investor_id == Users.id).first()
    if investor.is_investor == True:
    # product = Product.query.filter(Product.id == investor.id)
        return render_template('investor.html', user=investor)
    else:
        abort(403)
...
def create_investor():
    ...
    if 'loggedin' in session:
        investor_id = session['id']
        investor = Users.query.filter(Users.id == investor_id).first()
        if request.method == 'POST':
            age = int(request.form['age'])
            contact = request.form['contact']
            bio = request.form['bio']
            ...
            return redirect(url_for('get_investor'), investor_id=investor_id)
...
def delete_investor(investor_id):
    investor = Users.query.get_or_404(investor_id)
...
def get_trader(trader_id):
    traders = Users.query.get_or_404(trader_id)
